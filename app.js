document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayMenu(items) {
      menuContainer.innerHTML = items.map(generateMenuItemHTML).join("");
  }


    displayMenu(menu);

    // Filter functionality - Use correct selector for buttons
    document.querySelectorAll(".nav-link[data-filter]").forEach(button => {
      button.addEventListener("click", () => {
          ...
          if (filter === "all") {
              ...
              displayMenu(menu.filter(item => item.category === filter));
          }
      });
    });
  ...
  document.getElementById("search-input").addEventListener("input", (event) => {
    ...
    displayMenu(menu.filter(item => item.title.toLowerCase().includes(searchTerm)));
  });

  // Debugging overflow elements
